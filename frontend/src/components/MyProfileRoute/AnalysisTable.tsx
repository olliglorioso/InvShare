...
const AnalysisTable = ({
  ...
  getPrediction: (comp: string) => void;
        ...
        <TableBody>
          {analysisData.map((company: AnalysisData) => {
            ...
            if (correspondingPosition) {
              profitPercent = (
                (-1 +
                  company.sticks[company.sticks.length - 1].close /
                    (correspondingPosition.usersTotalOriginalPriceValue /
                ...
                <StyledTableRow key={`${company.name}`}>
                  <TableCell component="th" scope="row">
                    <Button
                      ...
                      onClick={() => getPrediction(company.name)}
                    >
                      {company.name}
                    </Button>
                  </TableCell>
