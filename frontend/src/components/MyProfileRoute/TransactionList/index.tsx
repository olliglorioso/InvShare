...
import { Collapse, Button, List, ListItemIcon, ListItemText } from "@material-ui/core";
...
const TransactionList = (props: {transactions: TransactionType[]}): JSX.Element => {
    ...
    return (
        <div style={{display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center"}}>
            <div>
                <h1 style={{fontWeight: "bold", fontSize: 25, fontFamily: "Arial", color: "black", textAlign: "center"}}>
                    Transactions
                </h1>
            </div>
            ...
            >   
                {props.transactions.map((transaction: TransactionType) => {
                    ...
                    return (
                        <div key={transaction.transactionDate} style={{width: "33%", textAlign: "center"}}>
                            <Button onClick={() => handleClick(transaction._id.toString())}>
                                ...
                                {(open as Record<string, boolean>)[transaction._id.toString()] ? <ExpandLess /> : <ExpandMore />}
                            </Button>
                            <Collapse
                                in={(open as Record<string, boolean>)[transaction._id.toString()]} 
                                timeout="auto"
                            >
                                <List component="div" disablePadding style={{display: "flex", alignItems: "center", flexDirection: "column"}}>
